...
// ===================== STORAGE =====================
function useStorage() {
  const KEY = "fundacion_afiliados";
  const SEQ_KEY = "fundacion_seq";
  ...
  const save = useCallback((afiliados) => {
    try { localStorage.setItem(KEY, JSON.stringify(afiliados)); } catch {}
  }, []);
    ...
    <div>
      ...
      <div style={S.grid(3)}>
        ...
        <Field label="Fecha de nacimiento">
          <Input type="date" value={form.nac_afiliado} onChange={e => set("nac_afiliado", e.target.value)} />
        </Field>
        <Field label="RH">
      ...
      <div style={S.grid(3)}>
        ...
        <Field label="Estado">
          <Select value={form.estado_afiliado} onChange={e => set("estado_afiliado", e.target.value)}>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 8, justifyContent: "flex-end" }}>
        ...
        <button style={S.btnPrimary} onClick={handleSubmit}>
          💾 {initial?.id_afiliado ? "Guardar cambios" : "Registrar afiliado"}
        </button>
      </div>
    </div>
  );
...
// ===================== VISTA CONSULTA =====================
function ConsultaView({ afiliados, onEdit, onDelete }) {
  ...
  const filtered = afiliados.filter(a => {
    ...
    const match = !q ||
      a.apellidos_afiliado?.toLowerCase().includes(q) ||
      a.nombres_afiliado?.toLowerCase().includes(q) ||
      a.doc_afiliado?.toLowerCase().includes(q) ||
      String(a.no_carnet_afiliado).includes(q);
    const eps = !filtroEps || String(a.id_eps) === filtroEps;
    const est = !filtroEstado || a.estado_afiliado === filtroEstado;
    const loc = !filtroLocalidad || String(a.id_barrio) === filtroLocalidad;
    return match && eps && est && loc;
  });

  const totalPages = Math.ceil(filtered.length / PER_PAGE);
  const paginated = filtered.slice((page - 1) * PER_PAGE, page * PER_PAGE);

  const activos = afiliados.filter(a => a.estado_afiliado === "ACTIVO").length;
  const inactivos = afiliados.filter(a => a.estado_afiliado === "INACTIVO").length;
  const conEPS = afiliados.filter(a => a.id_eps).length;
  ...
  return (
    <div>
      <div style={S.statsGrid}>
        {[
          { num: afiliados.length, label: "Total registrados", color: "#4f8ef7" },
          { num: activos, label: "Activos", color: "#4ade80" },
          { num: inactivos, label: "Inactivos", color: "#f87171" },
          { num: conEPS, label: "Con EPS registrada", color: "#a78bfa" },
        ].map(({ num, label, color }) => (
          <div key={label} style={S.statCard}>
            <div style={{ ...S.statNum, color }}>{num.toLocaleString("es-CO")}</div>
      ...
      <div style={S.card}>
        ...
        <div style={S.searchBox}>
          ...
          />
          <Select value={filtroEps} onChange={e => { setFiltroEps(e.target.value); resetPage(); }} style={{ width: 180 }}>
            <option value="">Todas las EPS</option>
            {EPS_LIST.map(e => <option key={e.id} value={e.id}>{e.nombre}</option>)}
          </Select>
          <Select value={filtroLocalidad} onChange={e => { setFiltroLocalidad(e.target.value); resetPage(); }} style={{ width: 160 }}>
            <option value="">Toda localidad</option>
            {LOCALIDADES.map(l => <option key={l.id} value={l.id}>{l.nombre}</option>)}
          </Select>
          <Select value={filtroEstado} onChange={e => { setFiltroEstado(e.target.value); resetPage(); }} style={{ width: 130 }}>
                ...
                >
                  ...
                  <td style={{ ...S.td, display: "flex", gap: 8 }}>
                    ...
                    <button style={S.btnDanger} onClick={() => {
                      if (window.confirm(`¿Eliminar a ${a.nombres_afiliado} ${a.apellidos_afiliado}?`))
                        onDelete(a.id_afiliado);
                    }}>🗑️</button>
                  </td>
                </tr>
              ))}
...
// ===================== VISTA REGISTRO =====================
function RegistroView({ onRegistrar }) {
...
export default function App() {
  ...
  const { getAll, save, nextId } = useStorage();
  ...
  const registrar = (data) => {
    const nuevo = { ...data, id_afiliado: nextId(), fecha_registro: new Date().toISOString().slice(0, 10) };
    const updated = [nuevo, ...afiliados];
    setAfiliados(updated);
    save(updated);
    setTab("consulta");
  };

  const editar = (actualizado) => {
    const updated = afiliados.map(a => a.id_afiliado === actualizado.id_afiliado ? actualizado : a);
    setAfiliados(updated);
    save(updated);
  };

  const eliminar = (id) => {
    const updated = afiliados.filter(a => a.id_afiliado !== id);
    setAfiliados(updated);
    save(updated);
  };
  ...
  return (
    <div style={S.app}>
      ...
      <main style={S.main}>
        ...
        {tab === "registro" && (
          <RegistroView onRegistrar={registrar} />
        )}
      </main>
    </div>
  );
